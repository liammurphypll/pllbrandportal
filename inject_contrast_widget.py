...
def build_widget_block(team_name, team_slug, section_number, logo_variants, bucket_colors):
  ...
  <div class="content" style="padding-top:0;">
    ...
    <style>
      ...
      .cw-tile {{ position: relative; border: 2px solid var(--rule); border-radius: 4px; cursor: pointer; overflow: hidden; aspect-ratio: 4 / 3; display: flex; align-items: center; justify-content: center; padding: 0.6rem; background: var(--ink); transition: border-color 0.15s ease, transform 0.15s ease; }}
    ...
    (function () {{
      ...
      function nearestBucket(hex) {{
        const rgb = hexToRgb(hex);
        let best = BUCKETS[0], bestDist = Infinity;
        BUCKETS.forEach(b => {{
          const brgb = hexToRgb(b.hex);
          const dist = (rgb.r - brgb.r) ** 2 + (rgb.g - brgb.g) ** 2 + (rgb.b - brgb.b) ** 2;
      ...
      function bestVariant(bgHex) {{
        ...
        candidates.forEach(v => {{
          const refHex = BUCKET_HEX[v.origBg] || BUCKET_HEX['navy'] || '#000000';
          const refRgb = hexToRgb(refHex);
          const dist = (rgb.r - refRgb.r) ** 2 + (rgb.g - refRgb.g) ** 2 + (rgb.b - refRgb.b) ** 2;
      ...
      function renderLogoGrid() {{
        el.logoGrid.innerHTML = LOGO_VARIANTS.map(v => {{
          ...
          return `<div class="cw-tile${{v.isFullColor ? ' full-color' : ''}}" data-variant="${{v.id}}" title="${{v.label}}">
