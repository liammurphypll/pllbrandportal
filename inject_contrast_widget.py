...
def build_widget_block(team_name, team_slug, section_number, logo_variants, bucket_colors):
  ...
  <div class="content" style="padding-top:0;">
    ...
    (function () {{
      ...
      // True if a bucket name resolves to a dark color
      const DARK_BUCKETS = new Set(
        BUCKETS.filter(b => luminance(b.hex) < 0.4).map(b => b.name)
      );

      // Picks the right 1-color variant purely from background luminance:
      // dark bg → dark-bkg variant; light bg → light-bkg variant.
      // Always constrained to activeCategory so the logo group never changes.
      function bestVariant(bgHex, category) {{
        const bgDark = luminance(bgHex) < 0.4;
        ...
        const preferred = pool.filter(v => v.origBg && (bgDark ? DARK_BUCKETS.has(v.origBg) : !DARK_BUCKETS.has(v.origBg)));
      ...
      const _init = bestVariant(currentHex(), null);
      activeCategory = _init.category;
      update();
    }})();
    </script>
    <!-- ═══ /AUTO-INJECTED ═══ -->
