...
def build_widget_block(team_name, team_slug, section_number, logo_variants, bucket_colors, html_dir=None):
  ...
  <div class="content" style="padding-top:0;">
    <section class="section">
      ...
      <div class="section-body" id="contrast-body">
        <div class="cw-widget" id="cwWidget">
          ...
          <div class="cw-right">
            <div class="cw-style-tabs" id="cwStyleTabs">
              <button class="cw-style-btn active" data-style="1-color">1-Color</button>
              <button class="cw-style-btn" data-style="full-color">Full Color</button>
            </div>
    ...
    <style>
      ...
      .cw-stage {{ position: relative; width: 100%; aspect-ratio: 1 / 1; border-radius: 6px; border: 1px solid var(--rule); display: flex; align-items: center; justify-content: center; transition: background-color 0.2s ease; padding: 2rem; overflow: hidden; box-sizing: border-box; }}
      ...
      .cw-style-tabs {{ display: flex; gap: 0.4rem; margin-bottom: 0.75rem; }}
      .cw-style-btn {{ flex: 1; padding: 0.4rem 0.6rem; font-family: 'JetBrains Mono', monospace; font-size: 0.55rem; letter-spacing: 0.15em; text-transform: uppercase; color: var(--gray-dim); background: none; border: 1px solid var(--rule); border-radius: 2px; cursor: pointer; transition: all 0.15s ease; }}
      .cw-style-btn:hover {{ color: var(--white); border-color: var(--gray-dim); }}
      .cw-style-btn.active {{ color: var(--black); background: var(--white); border-color: var(--white); }}
      ...
      .cw-tile {{ position: relative; border: 2px solid var(--rule); border-radius: 4px; cursor: pointer; overflow: hidden; aspect-ratio: 1 / 1; display: flex; align-items: center; justify-content: center; padding: 1rem; transition: border-color 0.15s ease, transform 0.15s ease; }}
      ...
      .cw-tile img {{ width: 100%; height: 100%; object-fit: contain; display: block; }}
    ...
    (function () {{
      ...
      let lockedVariant = null;   // non-null only when user manually picks a specific tile
      let activeCategory = null;  // constrains auto-pick to one logo group
      let activeStyle = '1-color'; // '1-color' | 'full-color'
      ...
      // Threshold 0.179 is the exact breakeven where white and black logos
      // both achieve the same contrast ratio against the background (≈ 4.5:1).
      // This implicitly handles saturation: a fully-saturated red has lum 0.213
      // (dark), yellow 0.928 (light), blue 0.072 (dark), matching human perception.
      function luminance(hex) {{
        const {{ r, g, b }} = hexToRgb(hex);
        const g2l = c => {{ c /= 255; return c <= 0.04045 ? c / 12.92 : Math.pow((c + 0.055) / 1.055, 2.4); }};
        return 0.2126 * g2l(r) + 0.7152 * g2l(g) + 0.0722 * g2l(b);
      }}
      const DARK_THRESHOLD = 0.179;
      // A background is only "dark enough for white logo" if it is dark by
      // BOTH WCAG luminance AND raw HSB brightness. This prevents saturated
      // pinks, oranges, and reds (low green channel → low WCAG lum, but
      // visually bright) from incorrectly triggering the white logo.
      const BRIGHT_CUTOFF = 0.60;  // HSB brightness above this → always light

      function isDarkBg(hex) {{
        const {{ r, g, b }} = hexToRgb(hex);
        const brightness = Math.max(r, g, b) / 255;
        return luminance(hex) < DARK_THRESHOLD && brightness < BRIGHT_CUTOFF;
      }}
      ...
      function isForDarkBg(v) {{
        ...
        // fall back to combined brightness + luminance check
        const hex = BUCKET_HEX[v.origBg];
        return hex ? isDarkBg(hex) : true;
      }}
      ...
      function bestFullColor(category) {{
        let pool = LOGO_VARIANTS.filter(v => v.isFullColor && (!category || v.category === category));
        if (pool.length === 0) pool = LOGO_VARIANTS.filter(v => v.isFullColor);
        return pool[0] || null;
      }}

      function setStyleTab(style) {{
        activeStyle = style;
      ...
      function renderLogoGrid() {{
        ...
        [...el.logoGrid.children].forEach(tile => {{
          tile.addEventListener('click', () => {{
            ...
            setStyleTab(clicked.isFullColor ? 'full-color' : '1-color');
            update();
          }});
        }});
      }}
      ...
      function setFromHex(hex) {{ hsb = rgbToHsb(hexToRgb(hex)); if (activeStyle !== 'full-color') lockedVariant = null; update(); }}
      ...
      function update() {{
        ...
        let active;
        if (activeStyle === 'full-color') {{
          // In full-color mode: honour a manually-locked FC tile, otherwise auto-pick
          const fcLocked = lockedVariant
            ? LOGO_VARIANTS.find(v => v.id === lockedVariant && v.isFullColor)
            : null;
          active = fcLocked || bestFullColor(activeCategory) || LOGO_VARIANTS.find(v => v.isFullColor) || LOGO_VARIANTS[0];
          lockedVariant = active.id; // keep locked so sliders don't clear it
        }} else {{
          // In 1-color mode: respect a manually-locked 1-color tile, otherwise auto-pick
          const oneLocked = lockedVariant
            ? LOGO_VARIANTS.find(v => v.id === lockedVariant && !v.isFullColor)
            : null;
          active = oneLocked || bestVariant(pickerHex, activeCategory);
        }}
        const isFullColor = active.isFullColor;

        // Full-color logos show on their own designed-for background
      ...
      el.hueSlider.addEventListener('input', () => {{ hsb.h = Number(el.hueSlider.value); if (activeStyle !== 'full-color') lockedVariant = null; update(); }});
      el.satSlider.addEventListener('input', () => {{ hsb.s = Number(el.satSlider.value); if (activeStyle !== 'full-color') lockedVariant = null; update(); }});
      el.briSlider.addEventListener('input', () => {{ hsb.b = Number(el.briSlider.value); if (activeStyle !== 'full-color') lockedVariant = null; update(); }});
      ...
      [...document.querySelectorAll('.cw-style-btn')].forEach(btn => {{
        btn.addEventListener('click', () => {{
          setStyleTab(btn.dataset.style);
          lockedVariant = null;
          update();
        }});
      }});
