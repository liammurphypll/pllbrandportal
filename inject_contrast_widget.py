...
def build_widget_block(team_name, team_slug, section_number, logo_variants, bucket_colors, html_dir=None):
  ...
  <div class="content" style="padding-top:0;">
    ...
    <style>
      .cw-widget {{ display: flex; gap: 2.5rem; align-items: stretch; }}
      .cw-left {{ flex: 0 0 auto; }}
      .cw-right {{ flex: 1; min-width: 0; display: flex; flex-direction: column; }}
      .cw-stage {{ position: relative; aspect-ratio: 1 / 1; height: 100%; width: auto; border-radius: 6px; border: 1px solid var(--rule); display: flex; align-items: center; justify-content: center; transition: background-color 0.2s ease; padding: 2rem; overflow: hidden; box-sizing: border-box; }}
      ...
      @media (max-width: 768px) {{ .cw-widget {{ flex-direction: column; align-items: stretch; }} .cw-left {{ flex: none; width: 100%; }} .cw-stage {{ height: auto; width: 100%; aspect-ratio: 1 / 1; }} }}
    </style>
